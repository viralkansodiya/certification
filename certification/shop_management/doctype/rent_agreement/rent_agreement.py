...
def update_agreement_status():
	...
	frappe.db.sql(f"""
				Update `tabRent Agreement` as ra, `tabShop Detail` as sd SET ra.status = 'Expired', sd.status = 'Available'
				Where ra.status = 'Active' and ra.docstatus=1  and ra.status != 'Cancelled' and  ra.shop_detail = sd.name and ra.end_date < '{current_date}'
			""", as_dict = 1)
